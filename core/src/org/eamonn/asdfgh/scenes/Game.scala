...
class Game extends Scene {
  ...
  def spawnNewInvader(version: invaderType): Unit = {
    if (spawnTimer <= 0f && resources >= version.tier) {
      resources -= version.tier
      invaders = Invader(Vec2(19f, 0f), version, this) :: invaders
      spawnTimer = 1f
    }
  }
  ...
  override def update(delta: Float): Option[Scene] = {
    ...
    rowThreat(i.location.x.floor.toInt) += i.version.tier
    })
